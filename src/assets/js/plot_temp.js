...
export default class Plotting {
    ...
    plot_general(data, layout_config = {}, animation = true) {
        console.log(data)
        if (animation) {
            var frame_num = 15
            var frames = this.gene_frame(data.trace, frame_num)

            
            data.trace[0].x = frames[1].data[0].x
            data.trace[0].y = frames[1].data[0].y
            ...
            layout = this.set_layout(layout)
            console.log("data",frames)
            // console.log("frames", frames[frame_num].data[0].x[frame_num])

            var div_name = this.div_name
            Plotly.newPlot(div_name, [data.trace[0]],
                layout, this.plotly_config
            ).then(function () {
            ...
            let layout = this.set_layout(layout_config)
    ...
    gene_frame(data, frame_num = 30) {
        ...
        for (var i = 0; i < frame_num; i++) {
            frames[i] = { data: [{ x: [], y: [] }] }
            frames[i].data[0].x = data[0].x.slice(0, (i + 1) * interval);
            frames[i].data[0].y = data[0].y.slice(0, (i + 1) * interval);
        }
        frames[frame_num] = { data: [{ x: [], y: [] }] }
        frames[frame_num].data[0].x = data[0].x;
        frames[frame_num].data[0].y = data[0].y;
        return frames
    }
}
